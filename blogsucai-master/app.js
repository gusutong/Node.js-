...
http.createServer((req, res) => {
    ...
    if (req.method == 'POST') {
        ...
        req.on('end', () => {
            ...
            if (data.username == 'admin' && data.pwd == 'admin')
                res.end(JSON.stringify({ status: 'OK', message: 'OK' }));
            else if (data.title != undefined && data.content != undefined) {
                datalist.push({ title: data.title, content: data.content });
                res.end(JSON.stringify({ status: 'OK', message: 'OK' }));
            }
        });
    }
    ...
    const pathname = parurl.pathname;
    const id = parurl.query.chapterId;
    ...
    if (pathname == '/favicon.ico' || pathname == '/listmanager/bg.jpg')
        res.end();

    if (pathname.slice(0, 6) == '/list/') {
        if (pathname == '/list/')
            fs.createReadStream(dir + 'chapterList.html').pipe(res);
        else
            fs.createReadStream(dir + pathname.slice(6, pathname.length)).pipe(res);
    } else if (pathname.slice(0, 8) == '/detail/') {
        if (pathname == '/detail/') {
            fs.createReadStream(dir + 'chapter.html').pipe(res);
        } else
            fs.createReadStream(dir + pathname.slice(8, pathname.length)).pipe(res);
    } else if (pathname.slice(0, 7) == '/login/') {
        if (pathname == '/login/') {
            fs.createReadStream(dir + 'login.html').pipe(res);
        } else
            fs.createReadStream(dir + pathname.slice(7, pathname.length)).pipe(res);
    } else if (pathname.slice(0, 13) == '/listmanager/') {
        if (pathname == '/listmanager/') {
            fs.createReadStream(dir + 'list.html').pipe(res);
        } else
            fs.createReadStream(dir + pathname.slice(13, pathname.length)).pipe(res);
    } else if (pathname.slice(0, 12) == '/addChapter/') {
        if (pathname == '/addChapter/') {
            fs.createReadStream(dir + 'addChapter.html').pipe(res);
        } else
            fs.createReadStream(dir + pathname.slice(12, pathname.length)).pipe(res);
    }
}).listen(8083);
...
http.createServer((req, res) => {
    ...
    var arr = [];
    if (id) {
        arr[0] = chapterList[id - 1];
        res.end(JSON.stringify(arr));
    }

    //console.log(datalist, pathname);
